...
class ProfileAdmin(admin.ModelAdmin):
    list_display = (
        'user', 'telephone_number', 'id_status', 'user_group')
    search_fields = ('telephone_number', 'user__username', 'user__groups__name')
    list_filter = ('telephone_number', 'user__groups',)

    def user_group(self, obj):
        return " - ".join([t.name for t in obj.user.groups.all().order_by("id")])
...
class ProductoAdmin(admin.ModelAdmin):
    list_display = (
        'id', 'nombreProducto', 'precio', 'id_status', 'date_created')
    list_filter = ('id_status',)
